  ...
  return (
    <div className="p-6 max-w-4xl">
      <div className="flex items-center gap-4 mb-6">
        <Link href="/admin/don-hang" className="text-neutral-500 hover:text-primary text-sm">← Quay lại</Link>
        <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="bg-white rounded-2xl p-5 shadow-card">
            <h2 className="font-heading font-bold text-base mb-4">Thông tin khách hàng</h2>
            <div className="grid grid-cols-2 gap-3 text-sm">
              ...
              <div className="col-span-2">
                <div className="text-neutral-500 text-xs">Địa chỉ giao hàng</div>
                <div className="font-semibold">{order.shipping_address}</div>
              </div>
              {order.note && (
                <div className="col-span-2">
                  <div className="text-neutral-500 text-xs">Ghi chú</div>
                  <div className="font-semibold">{order.note}</div>
                </div>
              )}
            </div>
          </div>
              ...
              {order.items.map((item) => (
                <div key={item.id} className="flex justify-between text-sm py-2 border-b border-neutral-50 last:border-0">
                  <div>
                    ...
                    {item.custom_note && <div className="text-xs text-primary">Note: {item.custom_note}</div>}
                    <div className="text-neutral-500">x{item.quantity}</div>
                  </div>
                  <div className="font-bold text-right">{formatPrice(Number(item.total_price))}</div>
                </div>
              ))}
            </div>
              ...
              {order.status_history.map((h, i) => (
                <div key={h.id} className="flex gap-3 text-sm">
                  <div className="flex flex-col items-center">
                    ...
                    {i < order.status_history.length - 1 && <div className="w-0.5 h-full bg-neutral-100 mt-1" />}
                  </div>
                  <div className="pb-3">
                    <div className="font-semibold">{statusLabels[h.new_status] || h.new_status}</div>
                    <div className="text-neutral-400 text-xs">{formatDate(h.created_at)} · {h.changed_by}</div>
        ...
        <div className="space-y-4">
          ...
          {order.payment && (
            <div className="bg-white rounded-2xl p-5 shadow-card">
              <h2 className="font-heading font-bold text-base mb-4">Thanh toán</h2>
              <div className="space-y-2 text-sm">
                ...
                {order.payment.bank_name && (
                  <div className="flex justify-between">
                    ...
                    <span className="font-semibold">{order.payment.bank_name}</span>
                  </div>
                )}
                {order.payment.paid_at && (
                  <div className="flex justify-between">
                    <span className="text-neutral-500">Thanh toán lúc</span>
                    <span className="font-semibold text-xs">{formatDate(order.payment.paid_at)}</span>
                  </div>
                )}
              </div>
            </div>
