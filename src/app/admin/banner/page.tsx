    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-2xl border border-neutral-200/80 shadow-sm">
        <div>
          ...
          <p className="text-neutral-500 text-sm">Thêm, sửa, sắp xếp các banner trượt trên màn hình chính của trang chủ.</p>
        </div>
      ...
      {successMsg && (
        <div className="bg-green-50 border border-green-200 text-green-700 px-4 py-3 rounded-xl text-sm font-semibold transition-all">
          ✨ {successMsg}
        </div>
      )}

      {error && (
        <div className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-xl text-sm font-semibold transition-all">
          ⚠️ {error}
        </div>
      )}
      ...
      {/* Banner List */}
      {isLoading ? (
        <div className="flex flex-col items-center justify-center py-20 space-y-3 bg-white rounded-2xl border border-neutral-200/80 shadow-sm">
          <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-primary"></div>
          <span className="text-sm text-neutral-500 font-medium">Đang tải dữ liệu banner...</span>
        </div>
      ) : banners.length === 0 ? (
        <div className="text-center py-16 bg-white rounded-2xl border border-neutral-200/80 shadow-sm space-y-4">
          <div className="bg-neutral-100 p-4 rounded-full inline-block text-neutral-400">
            <ImageIcon size={36} />
          </div>
          <div>
            <h3 className="font-bold text-neutral-800 text-lg">Chưa có banner nào</h3>
        ...
        <div className="grid grid-cols-1 gap-6">
          {banners.map((banner, index) => (
            <div 
              key={banner.id} 
              className={`bg-white border rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-all duration-200 flex flex-col lg:flex-row ${
                banner.status === 'hidden' ? 'border-neutral-200 opacity-75' : 'border-neutral-200'
              }`}
            >
              {/* Image Preview */}
              ...
              <div className="flex-1 p-6 flex flex-col justify-between space-y-4">
                  ...
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    ...
                    <div className="flex items-center gap-1">
                      <button 
                        ...
                        disabled={index === 0}
                        className="p-1.5 rounded-lg border hover:bg-neutral-50 disabled:opacity-30 disabled:pointer-events-none"
                        title="Di chuyển lên"
                      >
                        <ArrowUp size={14} />
                      </button>
                ...
                <div className="flex items-center justify-end gap-3 pt-4 border-t border-neutral-100">
                  ...
                  <button
                    ...
                    className="px-3 py-2 border border-primary text-primary hover:bg-primary-light rounded-xl text-xs font-semibold flex items-center gap-1.5 transition-all"
                  >
                    <Edit size={14} />
                    <span>Chỉnh sửa</span>
                  </button>
                  ...
                  <button
                    ...
                    className="px-3 py-2 border border-red-200 text-red-600 hover:bg-red-50 hover:border-red-300 rounded-xl text-xs font-semibold flex items-center gap-1.5 transition-all"
                  >
                    <Trash2 size={14} />
                    <span>Xóa</span>
                  </button>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 overflow-y-auto bg-black/60 backdrop-blur-[3px] flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-4xl h-[90vh] flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            {/* Modal Header */}
            ...
            <div className="flex-1 overflow-y-auto p-6 flex flex-col lg:flex-row gap-6">
              ...
              <form onSubmit={handleSubmit} id="banner-form" className="flex-1 space-y-5">
                    ...
                    <div className="relative rounded-2xl overflow-hidden border border-neutral-200 group aspect-video bg-neutral-100">
                      ...
                      <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                        <button 
                          ...
                          className="bg-white text-neutral-800 px-4 py-2 rounded-xl text-xs font-bold hover:scale-105 transition-transform"
                        >
                          Thay ảnh khác
                        </button>
                        <button 
                          ...
                          className="bg-red-500 text-white px-4 py-2 rounded-xl text-xs font-bold hover:scale-105 transition-transform"
                        >
                          Xóa ảnh
                        </button>
                      </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-bold text-neutral-700 uppercase tracking-wider mb-1">Tiêu đề (Title)</label>
                    <input 
                      name="title" 
                      value={form.title} 
                  ...
                  <div>
                    <label className="block text-xs font-bold text-neutral-700 uppercase tracking-wider mb-1">Tiêu đề nổi bật (Subtitle)</label>
                    <input 
                      name="subtitle" 
                      value={form.subtitle} 
                      onChange={handleChange} 
                ...
                <div>
                  <label className="block text-xs font-bold text-neutral-700 uppercase tracking-wider mb-1">Mô tả ngắn</label>
                  <textarea 
                    name="description" 
                    ...
                    className="w-full px-4 py-2 border border-neutral-200 rounded-xl focus:border-primary outline-none text-sm resize-none transition-all"
                  />
                </div>
                ...
                <div className="border-t border-neutral-100 pt-4 space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-neutral-50 p-4 rounded-2xl border border-neutral-200/50">
                    {/* Primary Button */}
                    <div className="space-y-3">
                      ...
                      <div>
                        ...
                        <input 
                          ...
                          className="w-full px-3 py-1.5 border border-neutral-200 bg-white rounded-lg focus:border-primary outline-none text-xs"
                        />
                      </div>
                      <div>
                        ...
                        <input 
                          ...
                          className="w-full px-3 py-1.5 border border-neutral-200 bg-white rounded-lg focus:border-primary outline-none text-xs font-mono"
                        />
                      </div>
                    </div>

                    {/* Secondary Button */}
                    <div className="space-y-3 border-t md:border-t-0 md:border-l md:pl-4 border-neutral-200/60">
                ...
                <div className="grid grid-cols-2 gap-4 border-t border-neutral-100 pt-4">
                  ...
                  <div>
                    <label className="block text-xs font-bold text-neutral-700 uppercase tracking-wider mb-1">Trạng thái</label>
                    <select 
                      name="status" 
                      value={form.status} 
                      onChange={handleChange} 
              ...
              <div className="w-full lg:w-96 flex flex-col space-y-4 flex-shrink-0">
                <div className="sticky top-0 space-y-3">
                  ...
                  <div className="bg-neutral-50 p-4 rounded-2xl border text-xs text-neutral-500 leading-relaxed">
                    💡 **Mẹo thiết kế:**
                    <ul className="list-disc pl-4 mt-1.5 space-y-1">
                      ...
                      <li>Chọn tone màu ảnh nền hài hòa với màu **Hồng Pastel (#e07a5f)** của website.</li>
                    </ul>
                  </div>
                </div>
              </div>
            ...
            {/* Modal Footer */}
            <div className="px-6 py-4 border-t bg-neutral-50 flex items-center justify-end gap-3">
