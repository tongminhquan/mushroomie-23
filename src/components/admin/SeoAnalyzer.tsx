...
export default function SeoAnalyzer({ form, setForm }: SeoAnalyzerProps) {
  ...
  useEffect(() => {
    ...
    if (!keyword) {
      ...
      setResults(checks); setScore(0); return
    }
    ...
    // 2. Keyword in meta description (10pts)
    ...
    // ── ADDITIONAL (30 pts) ─────────────────────
    // 7. Keyword in headings (5pts)
    const headings = (rawHtml.match(/<h[1-6][^>]*>([\s\S]*?)<\/h[1-6]>/gi) || [])
      .map(h => h.replace(/<[^>]+>/g, '')).join(' ').toLowerCase()
    if (headings.includes(keyword)) {
      pts += 5
      checks.push({ status: 'success', text: 'Đã tìm thấy từ khoá chính trong các tiêu đề phụ.', group: 'additional' })
    } else {
      checks.push({ status: 'warning', text: 'Nên thêm từ khoá vào ít nhất một tiêu đề phụ (H2, H3...).', group: 'additional' })
    }

    // 8. Keyword in image alt (5pts)
    ...
    if (altTexts.length > 0 && altTexts.includes(keyword)) {
      ...
      checks.push({ status: 'error', text: 'Không tìm thấy hình ảnh có thuộc tính alt. Hãy thêm hình và alt text.', group: 'additional' })
    }

    // 9. Keyword density (5pts)
    if (kwCount > 0 && wordCount > 0) {
      const density = (kwCount / wordCount) * 100
      const densityStr = density.toFixed(2)
      if (density >= 0.5 && density <= 2.5) {
        pts += 5
        checks.push({ status: 'success', text: `Mật độ từ khóa là ${densityStr}, từ khóa chính và sự kết hợp xuất hiện ${kwCount} lần.`, group: 'additional' })
      } else if (density > 2.5) {
    ...
    // 13. Keyword at start of title (5pts)
      ...
      <div className={`rounded-2xl border shadow-sm overflow-hidden ${scoreBg}`}>
        ...
        <div className="p-4">
          ...
          <div className="mt-3">
            ...
            <input
              type="text"
              placeholder="vd: vòng tay handmade cute"
      ...
      <div className="bg-white rounded-2xl border border-neutral-200 shadow-sm overflow-hidden">
        ...
        {activeTab === 'overview' && (
          <div className="p-4 space-y-4">
            ...
            <div className="flex items-center justify-between mb-1">
              <p className="text-xs font-bold text-neutral-500 uppercase tracking-wide">Xem trước</p>
            ...
            <div className={`border border-neutral-200 rounded-xl p-4 bg-white ${previewDevice === 'mobile' ? 'max-w-xs' : ''}`}>
              <p className="text-xs text-neutral-400 truncate mb-0.5">https://mushroomie.vn/{displaySlug}/</p>
            ...
            {/* Meta Description */}
            <div className="space-y-2 bg-neutral-50 rounded-xl p-3 border border-neutral-100">
              <LengthGauge value={metaLen} min={80} ideal={160} max={230} label="Thẻ mô tả (Meta Description)" />
              <textarea
        ...
        {activeTab === 'social' && (
          <div className="p-4 space-y-3">
            <p className="text-xs text-neutral-400">Cài đặt hiển thị khi chia sẻ lên mạng xã hội (Facebook, Twitter, Zalo...)</p>
            <div>
              <label className="block text-xs font-semibold text-neutral-600 mb-1">Tiêu đề OG (Open Graph)</label>
              <input placeholder="Để trống để dùng Tiêu đề SEO" className="w-full px-3 py-2 border border-neutral-200 rounded-lg text-sm focus:border-primary outline-none" />
            </div>
            <div>
              <label className="block text-xs font-semibold text-neutral-600 mb-1">Mô tả OG</label>
              <textarea rows={2} placeholder="Để trống để dùng Mô tả Meta" className="w-full px-3 py-2 border border-neutral-200 rounded-lg text-sm focus:border-primary outline-none resize-none" />
            </div>
          </div>
        )}
      </div>
