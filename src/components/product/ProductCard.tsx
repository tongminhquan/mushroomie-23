...
export default function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    <article className="theme-transition m-card m-glow group relative flex h-full flex-col overflow-hidden rounded-[24px] border-[1.5px] border-theme-border bg-theme-card hover:border-pink">
      <Link href={`/san-pham/${product.slug}`} onClick={handleSelectItem} className="m-card-media relative block aspect-[3/4] w-full shrink-0 overflow-hidden bg-theme-subtle">
        <SafeImage
          src={imageUrl}
          alt={product.name}
          imageKind="product"
