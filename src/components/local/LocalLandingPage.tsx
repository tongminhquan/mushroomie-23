  ...
  return (
    <div className="theme-transition min-h-screen bg-theme-page pb-16 text-theme-primary">
      ...
      <section className="brand-container mt-4">
        <div
          className="rounded-[24px] border-[1.5px] border-theme-border bg-theme-card p-6 shadow-card sm:p-9"
        >
          ...
          <h1 className="font-heading text-2xl leading-tight text-theme-primary md:text-4xl">{page.h1}</h1>
          <p className="mt-3 max-w-2xl text-[15px] leading-relaxed text-theme-secondary">{page.intro}</p>
      ...
      <section data-reveal className="brand-container mt-8" aria-labelledby="local-contact-heading">
        <div className="grid gap-5 rounded-[18px] border-[1.5px] border-theme-border bg-theme-card p-5 shadow-card sm:p-6 lg:grid-cols-[1.5fr_0.75fr_0.75fr]">
          <div>
            ...
            <ul className="mt-3 space-y-2 text-sm leading-relaxed text-theme-secondary">
          ...
          <div>
            ...
            <p className="text-xs leading-relaxed text-theme-muted">Nên liên hệ trước nếu bạn muốn hẹn nhận trực tiếp.</p>
          </div>
        </div>
      </section>
      ...
      <section data-reveal className="brand-container mt-8">
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          {page.highlights.map((h) => (
            <div key={h.title} className="rounded-[18px] border-[1.5px] border-theme-border bg-theme-card p-5 shadow-card">
              <div className="mb-2 text-2xl" aria-hidden>{h.emoji}</div>
              <h2 className="font-heading text-base text-theme-primary">{h.title}</h2>
              <p className="mt-1.5 text-sm leading-relaxed text-theme-secondary">{h.body}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section data-reveal className="brand-container mt-8" aria-labelledby="order-process-heading">
        <h2 id="order-process-heading" className="font-heading text-xl text-theme-primary">
        ...
        <ol className="mt-4 grid gap-4 sm:grid-cols-3">
          <li className="rounded-[18px] border-[1.5px] border-theme-border bg-theme-card p-5 shadow-card">
            <span className="flex h-10 w-10 items-center justify-center rounded-full bg-pink text-primary" aria-hidden>
              <Send size={18} />
            </span>
            <h3 className="mt-3 font-heading text-base text-theme-primary">1. Gửi ý tưởng</h3>
            <p className="mt-1.5 text-sm leading-relaxed text-theme-secondary">Cho {BRAND.name} biết màu, charm, size, dịp tặng và khoảng ngân sách bạn mong muốn.</p>
          </li>
          <li className="rounded-[18px] border-[1.5px] border-theme-border bg-theme-card p-5 shadow-card">
            <span className="flex h-10 w-10 items-center justify-center rounded-full bg-[#fff3cf] text-[#9b6500]" aria-hidden>
              <CheckCircle2 size={18} />
            </span>
            <h3 className="mt-3 font-heading text-base text-theme-primary">2. Chốt mẫu và chi phí</h3>
            <p className="mt-1.5 text-sm leading-relaxed text-theme-secondary">Hai bên xác nhận cách phối, nội dung chữ, thời gian hoàn thiện và tổng chi phí trước khi làm.</p>
          </li>
          <li className="rounded-[18px] border-[1.5px] border-theme-border bg-theme-card p-5 shadow-card">
            <span className="flex h-10 w-10 items-center justify-center rounded-full bg-[#eaf8ef] text-[#237a43]" aria-hidden>
              <PackageCheck size={18} />
            </span>
            <h3 className="mt-3 font-heading text-base text-theme-primary">3. Làm và giao sản phẩm</h3>
            <p className="mt-1.5 text-sm leading-relaxed text-theme-secondary">{BRAND.name} làm thủ công, kiểm tra chi tiết rồi hẹn nhận hoặc gửi qua đơn vị vận chuyển.</p>
          </li>
        </ol>
      </section>
      ...
      <section data-reveal className="brand-container mt-8">
        <div className="rounded-[18px] border-[1.5px] border-yellow/70 bg-theme-subtle p-5 sm:p-6">
          ...
          <p className="mt-2 text-sm leading-relaxed text-theme-secondary">{areaDelivery.pickup}</p>
          <p className="mt-2 text-sm leading-relaxed text-theme-secondary">
      ...
      {areaNote && (
        <section data-reveal className="brand-container mt-8" aria-labelledby="area-note-heading">
          <div className="rounded-[18px] border-[1.5px] border-theme-border bg-theme-card p-5 sm:p-6">
            <h2 id="area-note-heading" className="font-heading text-lg text-theme-primary">
              Điều nên biết khi đặt tại {page.area}
            </h2>
            <p className="mt-3 text-sm leading-relaxed text-theme-secondary">{areaNote}</p>
          </div>
        </section>
      )}
      ...
      {related.length > 0 && (
        <section data-reveal className="brand-container mt-8">
          <h2 className="font-heading text-lg text-theme-primary">Khu vực &amp; sản phẩm liên quan</h2>
      ...
      <section data-reveal className="brand-container mt-8" aria-labelledby="local-faq-heading">
        <h2 id="local-faq-heading" className="font-heading text-xl text-theme-primary">
          Câu hỏi thường gặp
        </h2>
        <div className="mt-4 divide-y divide-theme-border rounded-[18px] border-[1.5px] border-theme-border bg-theme-card px-5 shadow-card sm:px-6">
          {faqs.map((faq) => (
