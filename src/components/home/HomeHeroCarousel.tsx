        ...
        {banners.length > 1 && (
          <>
            ...
            <div className="absolute bottom-3 right-3 z-20 flex gap-1 rounded-lg bg-black/25 p-1 backdrop-blur-sm">
              {banners.map((banner, index) => (
                <button key={banner.id} onClick={() => setCurrent(index)} aria-label={`Chuyển đến banner ${index + 1}`} aria-current={index === current} className="grid h-11 min-w-11 place-items-center rounded-lg">
